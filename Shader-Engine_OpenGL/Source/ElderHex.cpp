...
void ElderHex::OnUpdate() {
    ...
    //bag.Transform->Rotation += glm::vec3( 0, 1 * DeltaTime, 0);
    MainCamera.fov += 1 * DeltaTime;
    bag.Update();
...
void ElderHex::OnRender() {
    ...
    //// Transform
    //objectModel = glm::translate(objectModel, glm::vec3(-2.0f, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
